...
function ActionPlan() {
  ...
  const ladder = recommendations.map((recommendation, index) => ({
    ...
    capacity:
      ...
      recommendations
        .slice(0, index + 1)
        .reduce((total, current) => total + current.saving, 0),
  }));
        ...
        <CardContent className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-4 p-5">
          <div className="min-w-0">
            ...
            <p className="mt-1 flex items-center gap-1.5 text-xs text-muted-foreground">
              ...
              {saved > 0
                ? `${saved}% freed from ${selected.length} action${
                    selected.length > 1 ? "s" : ""
                  }`
      ...
      <Card className="overflow-hidden rounded-2xl border-border shadow-soft">
        <CardHeader className="border-b border-border bg-primary text-primary-foreground">
          ...
          <p className="text-xs text-primary-foreground/70">
            See how each adjustment gradually brings your workload back into a
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="text-sm font-semibold">
                Check your group before delegating
              </p>

              <p className="mt-1 text-sm text-muted-foreground">
                Before assigning work to another member, review the Capacity
                Circle to see who currently has enough room to take on more
          ...
          return (
            <Card
              ...
              className={`rounded-2xl shadow-soft transition-colors ${
                active
                  ? "border-positive/50 bg-positive-soft/40"
                  : "border-border"
              }`}
            >
      ...
      <Card className="rounded-2xl border-border shadow-soft">
        ...
        <CardContent className="space-y-3">
          <p className="text-sm text-muted-foreground">
            Selecting all three recommended changes brings your projected
            capacity to {forecastCapacity - 29}%, back inside a range that is
            more realistic to sustain.
          </p>
          ...
          {!isWorkable ? (
            <p className="text-xs text-muted-foreground" role="status">
              Choose enough actions to bring the projection below 95%. The
              recommended plan reaches {forecastCapacity - 29}%.
            </p>
          ) : null}
        </CardContent>
      </Card>
...
function ImpactStep({
  ...
  return (
    ...
    >
      ...
      <p className="mt-1 pl-1 text-2xl font-bold tabular-nums">
        {value}%
      </p>
